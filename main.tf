...
# -----------------------------------------------
# Hardcode Environment = Azure in KAC ConfigMap
# This is always AKS so Environment is always Azure
# -----------------------------------------------
resource "kubectl_manifest" "falcon_kac_environment" {
  yaml_body = <<-YAML
apiVersion: v1
kind: ConfigMap
metadata:
  name: falcon-kac-environment
  namespace: falcon-kac
  labels:
    app: falcon-kac
    app.kubernetes.io/name: falcon-kac
data:
  __CS_CLOUD_PROVIDER: "azure"
  __CS_CLOUD_REGION: "${data.azurerm_kubernetes_cluster.aks.location}"
  __CS_CLUSTER_TYPE: "aks"
  __CS_ENVIRONMENT: "Azure"
YAML
